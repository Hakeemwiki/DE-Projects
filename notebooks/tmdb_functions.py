...
def fetch_movie_data(ids):
    ...
    session = requests.Session()     # Create a session object to persist settings (like retries) across requests

    # Define retry strategy: try up to 3 times with backoff, for specific error codes
    retry = Retry(total=3, # Maximum number of retry attempts
                  backoff_factor=0.3, # Time delay between retries: 0.3s, 0.6s, 1.2s..
                  status_forcelist=[429, 500, 502, 503, 504]) # Retry only on these HTTP status codes
    ...
    results = []
    # Loop through each movie ID provided
    for mid in ids:
        try:
            # Make an API call to fetch movie details (with credits) for the current movie ID
            r = session.get(url.format(id=mid, api=api), timeout=5)

            # Raise an exception for unsuccessful status codes (e.g., 404, 500)
            r.raise_for_status()

            # If the response is successful, append the JSON data to results
            results.append(r.json())
        except Exception as e:
            # Log a warning if the fetch fails for a particular movie ID
